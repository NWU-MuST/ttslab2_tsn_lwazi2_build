...
RUN ln -s $USERHOME/src/za_lex/data/tsn/g2p.translit.txt rules
RUN ln -s $USERHOME/src/za_lex/data/tsn/phonemeset.json
...
## BUILD VOICE WITH:
## `docker run -v $TTSLAB2_TSN_LWAZI2_BUILD/out:/mnt/ext ttslab2_tsn_lwazi2_build`
############################################################
WORKDIR $USERHOME

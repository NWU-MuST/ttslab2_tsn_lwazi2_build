...
MAIN_DIACRITISER = "main_diacritiser.pickle"
MAIN_POSTAGGER = "main_postagger.pickle"

def make_voice(langs, synthfile="frontend"):
    pronun = {}
    for i, lang in enumerate(langs):
        if i == 0:
            exec("from ttslab.lang.%(lang)s import NewNTTSVoice as Voice" % {"lang": lang})
            langpref = "main"
        else:
            langpref = lang
        pronun[langpref] = {}
    ...
    if synthfile == "frontend":
        ...
        voice.diacritiser = ttslab.fromfile(MAIN_DIACRITISER)
        voice.postag = ttslab.fromfile(MAIN_POSTAGGER)
